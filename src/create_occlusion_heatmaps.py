...
def do_occlusion_experiment(model, dataset, device, args):
    ...
    for i in range(n):
        ...
        np.save(args.save_path + f"/{name}_superpixel_water_mask_{n_segments}.npy", sp_map_water)     
        sp_map_land = generate_superpixel_occlusion_map(model, device, bands, mask=land_mask, superpixel_list=segments)
        np.save(args.save_path + f"/{name}_superpixel_land_mask_{n_segments}.npy", sp_map_land)
        sp_map_coastline = generate_superpixel_occlusion_map(model, device, bands, mask=coastline_mask, superpixel_list=segments)
        np.save(args.save_path + f"/{name}_superpixel_coastline_mask_{n_segments}.npy", sp_map_coastline)
        ...
        for pixel in water_pixels:
            ...
            np.save(args.save_path + f"/{name}_water_pixel_{pixel[0]}_{pixel[1]}_{patch_size}_{stride}.npy", map)

            sp_map = generate_superpixel_occlusion_map(model, device, bands, mask=mask, superpixel_list=segments)
            np.save(args.save_path + f"/{name}_superpixel_water_pixel_{pixel[0]}_{pixel[1]}_{n_segments}.npy", sp_map)
        
        for pixel in land_pixels:
            mask = mask_from_pixel(land_mask, pixel)
            map = generate_occlusion_map(model, device, bands, mask=mask, patch_size=patch_size, stride=stride)
            np.save(args.save_path + f"/{name}_land_pixel_{pixel[0]}_{pixel[1]}_{patch_size}_{stride}.npy", map)

            sp_map = generate_superpixel_occlusion_map(model, device, bands, mask=mask, superpixel_list=segments)
            np.save(args.save_path + f"/{name}_superpixel_land_pixel_{pixel[0]}_{pixel[1]}_{n_segments}.npy", sp_map)


        for pixel in coastline_pixels:
            mask = mask_from_pixel(coastline_mask, pixel)
            map = generate_occlusion_map(model, device, bands, mask=mask, patch_size=patch_size, stride=stride)
            np.save(args.save_path + f"/{name}_coastline_pixel_{pixel[0]}_{pixel[1]}_{patch_size}_{stride}.npy", map)

            sp_map = generate_superpixel_occlusion_map(model, device, bands, mask=mask, superpixel_list=segments)
            np.save(args.save_path + f"/{name}_superpixel_coastline_pixel_{pixel[0]}_{pixel[1]}_{n_segments}.npy", sp_map)
...
def generate_superpixel_occlusion_map(model, device, input_image, mask=None, superpixel_list=None):
    """
